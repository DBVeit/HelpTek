<template>
  <div class="component-main-container">
    <form @submit.prevent="onSelectParamRel">
      <div class="form-group">
        <label>Técnico: </label>
        <select v-model="RelatorioParam.tecnico_chamado">
          <option value="" disabled selected>Selecionar...</option>
          <option value="0">Crítica</option>
          <option value="1">Alta</option>
          <option value="2">Média</option>
          <option value="3">Baixa</option>
        </select>
      </div>
      <div class="form-group">
        <label>Status: </label>
        <select v-model="RelatorioParam.status_chamado">
          <option value="" disabled selected>Selecionar...</option>
          <option value="2">Em atendimento</option>
          <option value="3">Respondido</option>
          <option value="4">Concluido</option>
          <option value="0">Cancelado</option>
        </select>
      </div>
      <div class="form-group">
        <label>Data de criação: </label>
        Início
        <input type="date" v-model="RelatorioParam.dta_criacao_inicio" />
        Fim
        <input type="date" v-model="RelatorioParam.dta_criacao_fim" />
      </div>
      <div id="form-dash-submit" class="form-group-relatorio">
        <button class="submit-button-chamado">Executar</button>
      </div>
    </form>
    <div class="form-group-relatorio-acao" v-if="showResults">
      <div>
        <span>Total de resultados encontrados: {{ totalResults }}</span>
        <button class="bt-acoes-chamado" @click="generateExcelReport">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="16"
            height="16"
            fill="currentColor"
            class="bi bi-filetype-xlsx"
            viewBox="0 0 16 16"
          >
            <path
              fill-rule="evenodd"
              d="M14 4.5V11h-1V4.5h-2A1.5 1.5 0 0 1 9.5 3V1H4a1 1 0 0 0-1 1v9H2V2a2 2 0 0 1 2-2h5.5zM7.86 14.841a1.13 1.13 0 0 0 .401.823q.195.162.479.252.284.091.665.091.507 0 .858-.158.355-.158.54-.44a1.17 1.17 0 0 0 .187-.656q0-.336-.135-.56a1 1 0 0 0-.375-.357 2 2 0 0 0-.565-.21l-.621-.144a1 1 0 0 1-.405-.176.37.37 0 0 1-.143-.299q0-.234.184-.384.188-.152.513-.152.214 0 .37.068a.6.6 0 0 1 .245.181.56.56 0 0 1 .12.258h.75a1.1 1.1 0 0 0-.199-.566 1.2 1.2 0 0 0-.5-.41 1.8 1.8 0 0 0-.78-.152q-.44 0-.777.15-.336.149-.527.421-.19.273-.19.639 0 .302.123.524t.351.367q.229.143.54.213l.618.144q.31.073.462.193a.39.39 0 0 1 .153.326.5.5 0 0 1-.085.29.56.56 0 0 1-.255.193q-.168.07-.413.07-.176 0-.32-.04a.8.8 0 0 1-.249-.115.58.58 0 0 1-.255-.384zm-3.726-2.909h.893l-1.274 2.007 1.254 1.992h-.908l-.85-1.415h-.035l-.853 1.415H1.5l1.24-2.016-1.228-1.983h.931l.832 1.438h.036zm1.923 3.325h1.697v.674H5.266v-3.999h.791zm7.636-3.325h.893l-1.274 2.007 1.254 1.992h-.908l-.85-1.415h-.035l-.853 1.415h-.861l1.24-2.016-1.228-1.983h.931l.832 1.438h.036z"
            />
            />
          </svg>
        </button>
      </div>
    </div>
  </div>
</template>
<script>
import axios from "axios";

export default {
  name: "ChamadosPorTecnico",
  data() {
    return {
      RelatorioParam: {
        tecnico_chamado: "",
        status_chamado: "",
        dta_criacao_inicio: "",
        dta_criacao_fim: "",
      },
      totalResults: 0,
      showResults: false,
    };
  },
  methods: {
    onSelectParamRel() {
      let relatorio = new FormData();

      relatorio.append("tecnico_chamado", this.RelatorioParam.tecnico_chamado);
      relatorio.append("status_chamado", this.RelatorioParam.status_chamado);
      relatorio.append(
        "dta_criacao_inicio",
        this.RelatorioParam.dta_criacao_inicio
      );
      relatorio.append("dta_criacao_fim", this.RelatorioParam.dta_criacao_fim);

      axios
        .post(
          `http://localhost/projeto/helptek/php/api/functions/relatorios/getChamadosTecnico.php`,
          relatorio
        )
        .then((res) => {
          console.log("Server response:", res.data);
          if (res.data.error === true) {
            this.showAlertError(res.data.msg);
            this.showResults = false;
          } else {
            this.totalResults = res.data.total;
            this.showResults = true;
          }
        })
        .catch((err) => {
          console.log(err);
          this.showResults = false;
        });
    },
    generateExcelReport() {
      let relatorio = new FormData();

      relatorio.append("tecnico_chamado", this.RelatorioParam.tecnico_chamado);
      relatorio.append("status_chamado", this.RelatorioParam.status_chamado);
      relatorio.append(
        "dta_criacao_inicio",
        this.RelatorioParam.dta_criacao_inicio
      );
      relatorio.append("dta_criacao_fim", this.RelatorioParam.dta_criacao_fim);

      // Faz a solicitação para gerar o Excel
      axios({
        url: `http://localhost/projeto/helptek/php/api/functions/relatorios/generateChamadosExcel.php`,
        method: "POST",
        data: relatorio,
        responseType: "blob", // Importante para lidar com o arquivo binário
      })
        .then((response) => {
          const url = window.URL.createObjectURL(new Blob([response.data]));
          const link = document.createElement("a");
          link.href = url;
          link.setAttribute("download", "relatorio_chamados.xlsx"); // Nome do arquivo
          document.body.appendChild(link);
          link.click();
        })
        .catch((error) => {
          console.error("Erro ao gerar o relatório:", error);
        });
    },
  },
};
</script>
