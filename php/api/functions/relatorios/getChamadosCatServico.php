...
if (isset($_POST['cat_servico'])) {
    $cat_servico = $mysqli_con->real_escape_string($_POST['cat_servico']);
    ...
    list($sql_where, $params, $types) = montarWhere($cat_servico, $data_criacao_inicio, $data_criacao_fim);
    ...
    try {
        ...
        if (!empty($chamados)) {
            $spreadsheet = PlanilhaHelper::gerarPlanilha($chamados, "Chamados Serviço");
            PlanilhaHelper::salvarPlanilha($spreadsheet, "chamados_servico.xlsx");
        } else {
            $res['error'] = true;
...
function montarWhere($cat_servico, $data_criacao_inicio, $data_criacao_fim) {
    $sql_where = "WHERE 1=1";
    $params = [];
    $types = "";

    if ($data_criacao_inicio == "" && $data_criacao_fim == ""){
        $sql_where .= " AND chamados.id_categoria_servico = ?";
        $params[] = $cat_servico;
        $types .= 's';
    } else if ($data_criacao_inicio != "" && $data_criacao_fim != "") {
        $sql_where .= " AND chamados.id_categoria_servico = ? AND chamados.data_criacao BETWEEN ? AND ?";
        $params[] = $cat_servico;
        $params[] = $data_criacao_inicio;
        $params[] = $data_criacao_fim;
        $types .= 'sss';
    } else if ($data_criacao_inicio != "" && $data_criacao_fim == "") {
        $sql_where .= " AND chamados.id_categoria_servico = ? AND chamados.data_criacao >= ?";
        $params[] = $cat_servico;
        $params[] = $data_criacao_inicio;
        $types .= 'ss';
    } else if ($data_criacao_inicio == "" && $data_criacao_fim != "") {
