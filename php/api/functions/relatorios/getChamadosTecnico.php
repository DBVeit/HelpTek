...
if (isset($_POST['tecnico_chamado'])) {
    $tecnico_chamado = $mysqli_con->real_escape_string($_POST['tecnico_chamado']);
    ...
    list($sql_where, $params, $types) = montarWhere($tecnico_chamado, $data_criacao_inicio, $data_criacao_fim);
    ...
    try {
        ...
        if (!empty($chamados)) {
            $spreadsheet = PlanilhaHelper::gerarPlanilha($chamados, "Chamados tecnico");
            PlanilhaHelper::salvarPlanilha($spreadsheet, "chamados_tecnico.xlsx");
        } else {
            $res['error'] = true;
...
function montarWhere($tecnico_chamado, $data_criacao_inicio, $data_criacao_fim) {
    $sql_where = "WHERE 1=1";
    $params = [];
    $types = "";
    ...
    if ($data_criacao_inicio == "" && $data_criacao_fim == "") {
        $sql_where .= " AND chamados.id_user_tecnico = ?";
        $params[] = $tecnico_chamado;
        $types .= 's';
    } else if ($data_criacao_inicio != "" && $data_criacao_fim != "") {
        $sql_where .= " AND chamados.id_user_tecnico = ? AND chamados.data_criacao BETWEEN ? AND ?";
        $params[] = $tecnico_chamado;
        $params[] = $data_criacao_inicio;
        $params[] = $data_criacao_fim;
        $types .= 'sss';
    } else if ($data_criacao_inicio != "" && $data_criacao_fim == "") {
        $sql_where .= " AND chamados.id_user_tecnico = ? AND chamados.data_criacao >= ?";
        $params[] = $tecnico_chamado;
        $params[] = $data_criacao_inicio;
        $types .= 'ss';
    } else if ($data_criacao_inicio == "" && $data_criacao_fim != "") {
