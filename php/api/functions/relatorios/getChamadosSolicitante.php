...
if (isset($_POST['solicitante_chamado'])) {
    $solicitante_chamado = $mysqli_con->real_escape_string($_POST['solicitante_chamado']);
    ...
    list($sql_where, $params, $types) = montarWhere($solicitante_chamado, $data_criacao_inicio, $data_criacao_fim);
    ...
    try {
        ...
        if (!empty($chamados)) {
            $spreadsheet = PlanilhaHelper::gerarPlanilha($chamados, "Chamados Solicitante");
            PlanilhaHelper::salvarPlanilha($spreadsheet, "chamados_solicitante.xlsx");
        } else {
            $res['error'] = true;
...
function montarWhere($solicitante_chamado, $data_criacao_inicio, $data_criacao_fim) {
    $sql_where = "WHERE 1=1";
    $params = [];
    $types = "";

    if ($data_criacao_inicio == "" && $data_criacao_fim == ""){
        $sql_where .= " AND chamados.id_user = ?";
        $params[] = $solicitante_chamado;
        $types .= 's';
    } else if ($data_criacao_inicio != "" && $data_criacao_fim != "") {
        $sql_where .= " AND chamados.id_user = ? AND chamados.data_criacao BETWEEN ? AND ?";
        $params[] = $solicitante_chamado;
        $params[] = $data_criacao_inicio;
        $params[] = $data_criacao_fim;
        $types .= 'sss';
    } else if ($data_criacao_inicio != "" && $data_criacao_fim == "") {
        $sql_where .= " AND chamados.id_user = ? AND chamados.data_criacao >= ?";
        $params[] = $solicitante_chamado;
        $params[] = $data_criacao_inicio;
        $types .= 'ss';
    } else if ($data_criacao_inicio == "" && $data_criacao_fim != "") {
